/* jshint esversion: 6 */
/* jshint evil: true */
/* global MONO,Module,ENVIRONMENT_IS_NODE,ENVIRONMENT_IS_SHELL,ENVIRONMENT_IS_WEB, ENVIRONMENT_IS_WORKER */
...
var MonoSupportLib = {
	...
	$MONO: {
		 ...
		 * 
		 * @param {object} arg
		 * @param  {} {scheme=undefined
		 * @param  {} objectId=undefined
		 * @param  {} idArgs={}
		 * @param  {} props={}}
		 */
		_new_or_add_id_props: function ({ scheme = undefined, objectId = undefined, idArgs = {}, props = {} }) {
			console.log(`_new: s: ${scheme}, ${objectId}, idArgs: ${JSON.stringify(idArgs)}, props: ${JSON.stringify (props)}`);
		...
		_call_c_fn: function (fn, ...args) {
			MONO.var_info = [];
			fn (...args);
			const res = MONO.var_info;
			MONO.var_info = [];
			return res;
		},
		...
		_invoke_getter: function (objectIdStr, name) {
			...
			if (id.scheme == 'object') {
				...
				getter_res = this._call_c_fn (this.mono_wasm_invoke_getter_on_object, id.o, name);
				...
				getter_res = this._call_c_fn (this.mono_wasm_invoke_getter_on_value, dataHeap.byteOffset, id_props.klass, name);
		...
		_mono_wasm_add_getter_var: function(className, invokable) {
			let fixed_class_name = MONO._mono_csharp_fixup_class_name (className);
			if (invokable != 0) {
				var name;
		...
		_begin_value_type_var: function(className, args) {
			...
			this._hex_dump (this._base64_to_uint8 (base64String));
			const vt_obj = {
				value: {
					type            : "object",
					className       : fixed_class_name,
