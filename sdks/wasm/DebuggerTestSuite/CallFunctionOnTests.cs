...
namespace DebuggerTests
{
	...
	public class CallFunctionOnTests : DebuggerTestBase {
		...
		public static TheoryData<string, string, int, int, string, Func<string[], object>, string, bool> GettersTestData (string local_name, bool use_cfo)
			=> new TheoryData<string, string, int, int, string, Func<string[], object>, string, bool> {
		...
		public async Task PropertyGettersTest (string eval_fn, string method_name, int line, int col, string cfo_fn, Func<string[], object> get_args_fn, string local_name, bool use_cfo)
				...
				wait_for_event_fn: async (pause_location) => {
					...
					await CheckValue (res.Value ["result"], TArray ("System.DateTime[]", 2), $"{local_name}.DTArray");
					{
						...
						// FIXME: InvokeGetter on one of the elements
