...
namespace DebuggerTests
{
	...
	public class SourceList : DebuggerTestBase {
		...
		void CheckArray (JToken locals, string name, string class_name, string subtype=null) {
			Console.WriteLine ($"** Locals: {locals.ToString ()}");
			foreach (var l in locals) {
				if (name != l["name"]?.Value<string> ())
					continue;
				...
				var val = l["value"];
				Assert.Equal ("array", val ["type"]?.Value<string> ());
				Assert.Equal (class_name, val ["className"]?.Value<string> ());
				Assert.Equal (subtype, val ["subtype"]?.Value<string> ());
		...
		public async Task InspectLocalsWithDelegatesAtBreakpointSite () =>
			await CheckInspectLocalsAtBreakpointSite (
				...
				test_fn: (locals) => {
					...
					CheckFunction (locals, "fn_del_null", "Math/IsMathNull", subtype: "null");
					...
					CheckArray (locals, "fn_func_arr_unused", "System.Func`2<Math, bool>[]", subtype: "null");

					CheckFunction (locals, "fn_del_unused", "Math/IsMathNull", subtype: "null");
					CheckFunction (locals, "fn_del_null_unused", "Math/IsMathNull", subtype: "null");
					CheckArray (locals, "fn_del_arr_unused", "Math/IsMathNull[]", subtype: "null");
				}
			);
		...
		public async Task InspectLocalsWithGenericTypesAtBreakpointSite () =>
			await CheckInspectLocalsAtBreakpointSite (
				...
				test_fn: (locals) => {
					...
					CheckArray (locals, "list_arr_null", "System.Collections.Generic.Dictionary`2<Math[], Math/IsMathNull>[]", subtype: "null");
					...
					CheckArray (locals, "list_arr_unused", "System.Collections.Generic.Dictionary`2<Math[], Math/IsMathNull>[]", subtype: "null");
					CheckArray (locals, "list_arr_null_unused", "System.Collections.Generic.Dictionary`2<Math[], Math/IsMathNull>[]", subtype: "null");
				}
			);
